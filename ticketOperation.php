...
if (isset($_SESSION['loggedIn']) && $_SESSION['loggedIn'] === true) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['bus_code'], $_POST['date'])) {
        ...
        if (!$result) {
            ...
            echo "<p>No data found for the given date and bus code.</p>";
        }
        $stmt->close();
    }
...
<!-- Main Content -->
<main class="container mx-auto mt-8 bg-white p-6 rounded-md shadow-md">
    ...
    <form method="POST" action="" class="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
        <input type="text" name="bus_code" placeholder="Bus Code" 
            class="flex-1 px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-400" required>
        <input type="date" name="date" id="date" 
            class="flex-1 px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-400" required>
        <button type="submit" class="bg-red-500 text-white px-6 py-2 rounded-md hover:bg-red-600 transition">Search</button>
    </form>
                            ...
                            <td class="p-3 border border-gray-300">
                                <?php if ($seat['refund']) { ?>
                                    <form action="" method="POST">
                                        <button type="submit" name="refund" value="<?= htmlspecialchars($seat['seat_name']) ?>" 
                                            class="bg-red-500 text-white px-3 py-1 rounded-md hover:bg-red-600">Refund</button>
                                    </form>
                                <?php } else { ?>
                                    <button class="bg-gray-500 text-white px-3 py-1 rounded-md cursor-not-allowed" disabled>Not Refundable</button>
                                <?php } ?>
                            </td>
                        </tr>
                    <?php } ?>
